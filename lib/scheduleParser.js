...
.csv of full names dropped into public/uploads/schedule.csv via Tina
			|
			v
lib/scheduleParser.js reads .csv, returns 2D array of full names,
...
function parseSchedule() {
	...
	const scheduleCarrier = [
		...
		headerRow = fullArray[0],
		hourColumn = fullArray.map(row => row[0]).slice(1),
		specialtyShowIndices = [],
		showOnlyArray = fullArray.slice(1).map(row => row.slice(1)),
		idGrid = []
	]

	// search for specialty shows
