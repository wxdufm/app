...
const Banner = ({columns = [], aboveLogo = [], belowLogo = []}) => {
	...
	return (
		<div className="mx-auto mb-10 w-11/12 md:w-5/6 lg:w-full rounded-3xl bg-black p-4 text-white shadow-lg shadow-black/20">
			

			<div className="flex gap-4 items-stretch h-[45rem]">
				<div
					...
					{leftColumns.map((column, colIndex) => (
						<div key={colIndex} className="flex-1 flex flex-col gap-3">
							{column.images?.map((item, imgIndex) => (
								<div key={imgIndex} className="flex-shrink-0 overflow-hidden rounded-3xl bg-neutral-800">
									<img
										src={item.image}
				...
				<div
					className="flex-none w-[45%] grid overflow-hidden"
					style={{
				...
				>
						...
						{aboveLogo.length > 0 && (
							<div className="flex gap-3 w-full">
								{aboveLogo.map((column, colIndex) => (
									<div key={colIndex} className="flex-1 flex flex-col-reverse gap-3">
										{column.images?.map((item, imgIndex) => (
											<div key={imgIndex} className="flex-shrink-0 overflow-hidden rounded-3xl bg-neutral-800">
												<img src={item.image} alt={item.alt || `Image ${imgIndex + 1}`} className="w-full h-auto rounded-3xl" />
											</div>
										))}
									</div>
								))}
					...
					<div className="flex flex-col items-center py-2">
						...
						<h1 className="courier-prime w-full text-center text-sm md:text-xl lg:text-3xl mt-2">
